...
function generaFile(){
	    ...
	    if(!verificaDati()) 
			...
			if(ScreenTesto != ''){
				...
				//qualora non avessi gestit un $...$ lo pulisco. Tolgo eventuali doppi a capo
				ScreenTesto = ScreenTesto.replace(/\$[\w]+\$\W/, '').replaceAll(/\n[\w\W]?\n/g, '\n').replaceAll(/\n[\s]+\n/g, '\n');
				$('#txtTesto').val(ScreenTesto)
			}else{
				let imports = '';
				for (let i = 0; i<=4; ++i){
				...
				//qualora non avessi gestit un $...$ lo pulisco
				BrowseTesto = BrowseTesto.replace(/\$[\w]+\$\W/, '').replaceAll(/\n[\w\W]?\n/g, '\n').replaceAll(/\n[\s]+\n/g, '\n');
