...
function Ricerca(){	
	...
	if($('#txtCerca').val() == '' && $('.evidenziato').length == 0){		
		crea();
	}
	else if($('#txtCerca').val() == '' && $('.evidenziato').length > 0){		
		...
		for(let i=1; i<array.length; ++i){		
			let Regex = new RegExp($('#txtCerca').val(), 'g')		
			if(Regex.test(array[i])){			
				...
				let new_str = `<b>${i})</b> ` + aux_str.replaceAll($('#txtCerca').val(), '<b class="evidenziato">' + $('#txtCerca').val() +'</b>');
				$(`#sp${i}`).html(new_str)					
			}
		}	
	} 
...
function Ricerca2(){debugger;
	...
	for(let i=0; i<arrTrovati.length; ++i){
		...
		let Regex = new RegExp($('#txtCerca2').val(), 'g')		
		if(Regex.test(array[arrTrovati[i]])){	
			...
			let new_str = `<b>${i})</b> ` + aux_str_postRicerca1.replaceAll($('#txtCerca2').val(), '<b class="evidenziato2">' + $('#txtCerca2').val() +'</b>');
